...
docker-compose run --user 33:33 --rm wpcli_stg --path=stg --url=http://${IP}:${WP_PORT_stg} plugin uninstall --all
docker-compose run --user 33:33 --rm wpcli_prd --path=prd --url=http://${IP}:${WP_PORT_prd} plugin uninstall --all

echo "--  wpcli plugin install --" >> $logfile
# Install and list Plugins (online)

if curl -s --head  --request GET https://wordpress.com | grep "HTTP/2 200" > /dev/null; then 
   for name in $(cat plugins.txt); do 
    echo "Installing Plugin " $name #>> $logfile
    docker-compose run --user 33:33 --rm wpcli_stg --path=stg --url=http://${IP}:${WP_PORT_stg} plugin install $name --activate 
    docker-compose run --user 33:33 --rm wpcli_prd --path=prd --url=http://${IP}:${WP_PORT_prd} plugin install $name --activate 

done
else
   echo "--  i am not online  --" >> $logfile
fi
...
docker-compose run --user 33:33 --rm wpcli_stg --path=stg --url=http://${IP}:${WP_PORT_stg} theme delete --all 
docker-compose run --user 33:33 --rm wpcli_prd --path=prd --url=http://${IP}:${WP_PORT_prd} theme delete --all 
...
for name in $(cat 'input-data/themes/themes-local.txt'); do 
    #docker exec -it docker-wp-test-wpcli bash -c "wp theme install /mnt/input-data/themes/$name.zip" >> $logfile
    echo "--   install theme $name.zip --" >> $logfile
    
    docker-compose run --user 33:33 --rm wpcli_stg --path=stg --url=http://${IP}:${WP_PORT_stg} theme install /mnt/input-data/themes/$name.zip 
    docker-compose run --user 33:33 --rm wpcli_prd --path=prd --url=http://${IP}:${WP_PORT_prd} theme install /mnt/input-data/themes/$name.zip
...
###### nwoks onli if internet connected
